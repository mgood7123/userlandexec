...
bfd_fopen (const char *filename, const char *target, const char *mode, int fd)
{
  ...
  {
    a = testA(filename, target, mode, fd, 0, reta);
    b = testA(filename, target, mode, fd, 1, retb);
    c = testB(filename, target, mode, fd, 0, retc);
    d = testB(filename, target, mode, fd, 1, retd);
    ...
    if ( reta == 5 && retb == 5 && retc == 5 && retd == 5) {
        ...
        nbfd->iostream = _bfd_real_fopen_TMP (filename, mode, 1);
    } else {
        fprintf(stderr, "cannot map %s, using fopen instead\n", filename);
        nbfd->iostream = _bfd_real_fopen_TMP (filename, mode, 0);
  }
  }
...
//   return bfd_fopen_ (filename, target, FOPEN_RB, -1, 0);
}

/* Main program of GNU linker.
   ...
   attempt to replace fopen calls in bfd____bfdio.c and if possible use fmemopen (produces *FILE) or mmap, fmemopen would be prefered but might not be compatible with the BFD version of its FILE*
   
   priority, make dlopen mmapped and get it to work on a statically linked executable
   
   dlopen is primarily used with dlsym to obtain symbols from external files for use as if they where part of the program calling dlsym
...
void show_args______(int * argc, char * argv[]) { // shift function modified for this purpose
    for(int i=0; i<99; i++) {
        if (argv[i] == NULL) {
            printf("\nend of argv\n");
            break;
...
main (int argc, char **argv)
{
  ...
  program_name = argv[0];
  printf("xmalloc_set_program_name\n");
  xmalloc_set_program_name (program_name);

  printf("START_PROGRESS\n");
  ...
  if (saved_script_handle == NULL)
    {
      ...
      if (isfile) {
    printf("possibly opening linker script:\n%s\n", s);
